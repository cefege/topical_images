...
import re
import tempfile
import io  # Import io to handle byte operations
...
def main():
    global background_img_path, overlay_img_path, output_image_path, font_path, overlay_position

    st.set_page_config(layout="wide")
    st.title("Streamlit App with Multiple Image Uploads and Output")

    title = st.text_input("Image Title:", "")

    # Text area to take user input
    input_string = st.text_area("Enter list text here", "")

    # Button to confirm input
    confirm_button = st.button("Confirm")

    if confirm_button:
        if st.session_state.background_img_path == "":
            name_list = process_input(input_string)
            pil_img = transparent(st.session_state.overlay_img_path, output_image_path, overlay_position, text_list=name_list, font_path=font_path, title=title, text=title)
            st.session_state.output_img = pil_img
        else:
            name_list = process_input(input_string)
            pil_img = overlay_images(st.session_state.background_img_path, st.session_state.overlay_img_path, output_image_path, overlay_position, text_list=name_list, font_path=font_path, title=title, text=title)
            st.session_state.output_img = pil_img  # Store the output image in session state

    # Divide the app into three columns
    left_column, middle_column = st.columns(2)

    # Left column for frame image
    with left_column:
        st.header("Frame Image")
        frame_uploaded_file = st.file_uploader("Upload a frame image...", type=["jpg", "png", "jpeg"])
        if frame_uploaded_file is not None:
            # Save the uploaded file to a temporary directory
            with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
                tmp_file.write(frame_uploaded_file.read())
                overlay_img_path = tmp_file.name
                st.session_state.overlay_img_path = overlay_img_path

            frame_image = Image.open(overlay_img_path)
            new_image = frame_image.resize((100, 60))
            st.image(new_image, caption='Frame Image')

    # Middle column for pool image
    with middle_column:
        st.header("Background Image")
        pool_uploaded_file = st.file_uploader("Upload a pool image...", type=["jpg", "png", "jpeg"])
        if pool_uploaded_file is not None:
            # Save the uploaded file to a temporary directory
            with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
                tmp_file.write(pool_uploaded_file.read())
                background_img_path = tmp_file.name
                st.session_state.background_img_path = background_img_path
                print(True)

            pool_image = Image.open(background_img_path)
            new_image = pool_image.resize((100, 60))
            st.image(new_image, caption='Pool Image')
        else:
            st.session_state.background_img_path = ""
    ...
    if "output_img" in st.session_state:
        ...
        st.image(st.session_state.output_img, caption='Output Image', use_column_width=True)
        st.markdown("</div>", unsafe_allow_html=True)

        # Convert the PIL image to bytes for download
        img_bytes = io.BytesIO()
        st.session_state.output_img.save(img_bytes, format='PNG')
